...
function generate_new_playlist_item(){
    ...
    playlist_song.appendChild(removeItem);
    ...
    removeItem.addEventListener('click', (e) => {
        ...
        document.querySelector('.right_container h3').innerText = 'Total: '+titles.length;
    ...
    playlist_song.addEventListener('click', () => {
        if(!rmClick){
            const name = playlist_song.querySelector('.playlist-song-title h4').innerText;
            play(name);
        }
        else{
            rmClick = false;
...
function play(title, check=true){
    if(video.src && video.src != "http://localhost:800/" && (!video.src.includes("file://") || video.src.includes("blob")) && check){
        if(document.querySelector('.playing')){
            try{
                let p_top = document.querySelector('.playing').offsetTop-100;
                document.querySelector('.playing').classList.remove('playing');
                document.querySelectorAll('.playlist-song')[titles.indexOf(title)].classList.add('playing');
                document.querySelector('.playlist-container').scrollTo(p_top,document.querySelector('.playing').offsetTop-100);
            }
            catch{
            ...
            document.querySelectorAll('.playlist-song')[titles.indexOf(title)].classList.add('playing');
            document.querySelector('.playlist-container').scrollTo(0,document.querySelector('.playing').offsetTop-100);
        }
    }
    else if(!check){
        document.querySelectorAll('.playlist-song')[titles.indexOf(title)].classList.add('playing');
...
video.addEventListener('loadedmetadata', ()=>{
    document.querySelectorAll('.time-d')[0].innerHTML = '/'+convert_time(video.duration);
    document.querySelectorAll('.time-d')[1].innerHTML = '/'+convert_time(video.duration);
...
file_browser.addEventListener('change', () => {
    ...
    if((!video.src || video.src == "http://localhost:800/" || (video.src.includes("file://") && video.src.includes("index.html"))) && titles.length > 0){
        play(titles[0]);
...
search_box.addEventListener('keydown', (e)=>{
    if (e.key == "ArrowUp") {
        ...
        {
            i= i-1;
            searchResult_c.scrollTo(document.querySelectorAll('.search-result')[i+1].offsetTop,document.querySelectorAll('.search-result')[i].offsetTop-50);
        }
    ...
    if (e.key == "ArrowDown") {
        ...
        if(i+1 < document.querySelectorAll('.search-result').length){
            i= i+1;
            if(i > 2){
                searchResult_c.scrollTo(document.querySelectorAll('.search-result')[i-1].offsetTop+40,document.querySelectorAll('.search-result')[i].offsetTop-20);
            }
        }
